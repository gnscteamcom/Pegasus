<?php
		...
		if(!empty($_GET['Search'])){
			...
			$Pegar = @mysql_query("SELECT * FROM Embeds WHERE type='film' AND Title LIKE '%$Procurar%' ORDER BY PublishDate DESC LIMIT $Inicio,$LimitPerPag ") or die(mysql_error());
			$TotalLinhas = @mysql_query("SELECT * FROM Embeds WHERE type='film' AND Title LIKE '%$Procurar%'") or die(mysql_error());
		}else{
			$Pegar = @mysql_query("SELECT * FROM Embeds WHERE type='film' ORDER BY PublishDate DESC LIMIT $Inicio,$LimitPerPag ") or die(mysql_error());
			$TotalLinhas = @mysql_query("SELECT * FROM Embeds WHERE type='film'") or die(mysql_error());
		}
		...
		<div class="row">
                <div class="col-md-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">Lista de todos os Filmes <small><?php if(!empty($_GET['Search'])) echo 'Search: '.$_GET['Search']?></small></div>
                            ...
                            <div class="table-responsive">
									...
									if($CONFIG['BaseURI'] != ''){
                                        ...
                                        <tr class="">
											...
											<td class="center" style="width: 130px;">
												<a href="<?php echo adminURL('editstream'); ?>?id=<?php echo $linha['id']; ?>" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i> Editar</a>
								...
								<script data-cfasync="false">
									function AddDelete(ID){
										document.getElementById("TagADelete").href="<?php echo adminURL('editstream'); ?>?id="+ID+"&mod=delete";
									}
									...
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<div class="modal-footer">
												...
												<a id="TagADelete" type="button" class="btn btn-primary btn-danger" href="" ><i class="fa fa-trash-o"></i> Excluir Filme agora</a>
											</div>
										</div>
									</div>
								</div>
