$(function () {
    ...
    $('#item_desconto').mask('000.000.000.000.000,00', {
        reverse: true
    });
    $('#item_preco').mask('000.000.000.000.000,00', {
        reverse: true
    });
    ...
    //add Attr
    $('#btn_attr_add').live('click', function () {
        var atributo_id = $('#attr_add option:selected').val();
        var item_id = $('#item_id').val();
        var url = baseUri + '/admin/item/attr_add/';
        var dados = {
        ...
        if (atributo_id >= 1) {
            $.post(url, {
                ...
                window.location = baseUri + '/admin/item/editar/' + item_id + '/attr/';
            });
        }
    });
    //event change
            ...
            $.getJSON(url, {
                cat: cat
                    ...
                    function (data) {
                        ...
                        if (data != 0) {
                            $('<option>').val('').text('Selecione uma subcategoria...').appendTo('#item_sub');
    ...
    var popcontentItem = '<p>Código de referência interna do Produto. Ex: 001, TV3D008<p>';
    $('#item_ref').popover({
        placement: 'right',
    ...
    $('#item_estoque').live('change', function () {
        var item_id = $.trim($('#item_id').val());
        var item_estoque = $.trim($('#item_estoque').val());
        var url = baseUri + '/admin/item/estoqueUpdate/';
        $.post(url, {
            item_id: item_id,
